...
void cache::_remove_outdated_messages() {
    ...
    for( ; cache::CACHED_MESSAGE_LIVE_TIME < (unsigned int) (now - i->first); ++i) {
...
void cache::_remove_outdated_eebl() {
    ...
    for( ; cache::CACHED_EEBL_ID_LIVE_TIME < (unsigned int) (now - i->first ) && i != this->_eebl_cache_index.end(); ++i)
